...
- ✅ **Standalone theme item** — New `/r/ononc-theme.json` (registry:theme, 0 component files) installs only tokens: `npx shadcn@latest add https://dev.ononc.com/r/ononc-theme.json` for projects that want to share tokens without adding a component
- ✅ **Registry count** — out/r now has 337 JSON files (336 component items + ononc-theme.json), up from 336
- ✅ **Gate verification** — tsc 0 errors; eslint 0 errors (6 pre-existing text/* warnings); vitest 72 files / 261 tests (2 new test assertions: badge carries cssVars+css, EVERY item has cssVars+css); next build exit 0; out/r 337, all shadcn alias placeholders correct; component counts remain 336
- ✅ **Caveat documented** — Theme ships generic token names (background, foreground, border, muted, ring); installing into projects that already define them overrides those values — reconcile/scope in your design system as needed
